...
bool SolveSodoku(uint8_t board[BOX_GRID][BOX_GRID], uint16_t* rowMask, uint16_t* colMask, uint16_t* boxMask, coords* empties, size_t counter){
    ...
    size_t c       = SIZE_MAX;
    size_t idx     = SIZE_MAX;
    uint8_t minOps = UINT8_MAX;
    for(size_t i = 0; i < counter; i++){
        if(empties[i].col != SIZE_MAX){                                                 // && empties[i].row != SIZE_MAX also dont need to check this since both of them will be modified at the same time
            size_t row = empties[i].row;
            size_t col = empties[i].col;
            uint16_t used = rowMask[row] | colMask[col] | boxMask[BOX_INDEX(row, col)]; // we define variables to reduce the amount of deref which is expensive
            uint8_t Ops = 9 - __builtin_popcount(used);
            if(Ops < minOps){
                minOps = Ops;
                r = row;
                c = col;
    ...
    found:
    if(r == SIZE_MAX) // we dont need to check if c == SIZE_MAX since both are modified
        return true;
